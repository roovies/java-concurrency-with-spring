...
public class ProblematicStockServiceTest {

    private final ProblematicStockService problematicStockService = new ProblematicStockService();
    ...
    void 레이스_컨디션_문제_재현() throws InterruptedException {
        ...
        problematicStockService.initializeStock(1L, productName, initialQuantity);
        ...
        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    // 3. 재고 1개씩 감소
                    problematicStockService.decreaseStock(productName, 1);
                } catch (Exception e) {
        ...
        int finalQuantity = problematicStockService.getCurrentQuantity(productName);
        // 최종 재고 확인
