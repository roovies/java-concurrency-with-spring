...
public class InMemoryRaceConditionTest {
    ...
    void 레이스_컨디션_문제_재현() throws InterruptedException {
        ...
        stockService.initializeStock(1L, productName, initialQuantity);
        ...
        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    // 3. 재고 1개씩 감소
                    stockService.decreaseStock(productName, 1);
                } catch (Exception e) {
        ...
        int finalQuantity = stockService.getCurrentQuantity(productName);
        // 최종 재고 확인
    ...
    void 메서드에_synchronized를_적용하면_성능은_느리지만_메서드_호출시에_락이_걸려_레이스_컨디션을_방지한다() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    // 3. 재고 1개씩 감소
                    stockService.decreaseStockWithMethodSynchronized(productName, 1);
                } catch (Exception e) {
                    System.err.println("예외 발생: " + e.getMessage());
                } finally {
                    // 4. 쓰레드 작업이 끝나면 latch 1 감소시킴
    ...
    void 객체_단위_synchronized를_적용하면_객체에만_락이_걸려_성능을_개선하고_레이스_컨디션을_방지한다() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    // 3. 재고 1개씩 감소
                    stockService.decreaseStockWithObjectSynchronized(productName, 1);
                } catch (Exception e) {
                    System.err.println("예외 발생: " + e.getMessage());
                } finally {
                    // 4. 쓰레드 작업이 끝나면 latch 1 감소시킴
        ...
        List<CompletableFuture<Void>> futures =
                IntStream.range(0, threadCount)
                        .mapToObj(num -> CompletableFuture.runAsync(() -> {
                            try {
                                stockService.decreaseStockWithMethodReentrantLock(productName, 1);
                            } catch (Exception e) {
                                System.err.println("예외 발생: " + e.getMessage());
                            }
                        }))
    ...
    void ReentrantLock을_객체별로_적용하면_동일_객체에만_락이_걸리고_병렬성이_개선된다() throws InterruptedException {
        ...
        List<CompletableFuture<Void>> futures =
                IntStream.range(0, threadCount)
                        .mapToObj(num -> CompletableFuture.runAsync(() -> {
                            try {
                                stockService.decreaseStockWithObjectReentrantLock(productName, 1);
                            } catch (Exception e) {
                                System.err.println("예외 발생: " + e.getMessage());
                            }
                        }))
