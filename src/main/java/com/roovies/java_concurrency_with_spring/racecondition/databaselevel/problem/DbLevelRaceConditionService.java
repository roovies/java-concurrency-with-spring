...
public class DbLevelRaceConditionService {
    ...
    public void decreaseStock(String productName, int amount) {
        ...
        DbLevelRaceConditionStock dbLevelRaceConditionStock = stockRepository.findByProductName(productName)
        ...
        dbLevelRaceConditionStock.decrease(amount);

        // 3. 감소된 재고 저장
        // 나중에 실행된 트랜잭션이 먼저 실행된 트랜잭션의 결과를 덮어쓰게 됨
        stockRepository.save(dbLevelRaceConditionStock);
    }
    ...
    public void initializeStock(String productName, int quantity) {
        DbLevelRaceConditionStock dbLevelRaceConditionStock = new DbLevelRaceConditionStock(productName, quantity);
        stockRepository.save(dbLevelRaceConditionStock);
    }
